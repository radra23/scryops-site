...
GDPR Article 5 defines the principles that apply to all personal data processing, including telemetry:

**Lawfulness** — collection must have a documented legal basis: consent, contract fulfilment, legitimate interest, or one of the Article 6 alternatives. "It's useful for debugging" is not a legal basis.
...
**Security** — apply appropriate technical measures: access controls, encryption in transit and at rest, audit logging on access to sensitive data. "Appropriate" is calibrated to risk, not to convenience.
...
**Technical Data** — metrics, traces, and logs about system behaviour — is typically not personal unless it carries user-identifying attributes. Handle with care; verify absence of embedded PII before treating it as non-personal.
...
GDPR Articles 15–20 give individuals four rights over their personal data. Your telemetry pipeline must be able to respond to all of them:
...
**Erasure (Article 17)** — the "right to be forgotten." Any personal data in your telemetry that wasn't anonymised at ingest must be locatable and deletable on request. This is why hashing at the Collector and purging all direct identifiers is strongly preferable to trying to find and delete them later: if no direct identifier ever enters the backend, there's nothing to erase.
...
| Lawful basis | Is processing documented with a legal basis? | Maintain a data processing register per pipeline |
